...
export class ImageService {
  ...
  generateArt(seedString: String, artConfig: any, uploadCallBack: Function) {
    const config = {
    ...
    p5Import.registerMethod("uploadFunction", (val: String) => {uploadCallBack(val)});
    p5Import.createSketch(this.sketch);
  }
  ...
  function shuffleArray(array: any[]) {
    ...
    p5.draw = () => {
      if (p5.frameCount > 1500) {
        p5.noLoop();
        p5.saveCanvas(canvas, `${IMAGE_FILE_LOCATION}/${Cfg.STRING_SEED}`, "png").then(() => {
