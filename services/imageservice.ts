...
export class ImageService {
  ...
  generateArt(seedString: String, artConfig: any): String {
    const config = {
    ...
    /* Generates ART and returns filename of the saved art (save it /tmp) */
    let p5Instance = p5Import.createSketch(sketch);
    return `${seedString}.png`;
    // generate art
  }
}
...
function shuffleArray(array: any[]) {
  ...
  p5.draw = () => {
    if (p5.frameCount > 1500) {
      p5.noLoop();
      p5.saveCanvas(canvas, Cfg.STRING_SEED, "png").then(() => {
        console.log(`Image saved!! as ${Cfg.STRING_SEED}.png`);
      });
    }
    ...
    for (var i = 0; i < max; i++) {
      ...
      points[i].add(p5.createVector(angle_func_a(angle), angle_func_b(angle)));
