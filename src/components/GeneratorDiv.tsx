...
class GeneratorDiv extends React.Component<{
  ...
  constructor(props: any) {
    ...
    this.p5Instance = null; // Initialize as null
    const seedrandom = require("seedrandom");
    this.SEED_STRING = props.SEED_STRING;
    this.SEED_NUMBER = seedrandom(props.SEED_STRING + "\0").int32();
    this.RESOLUTION = parseInt(props.RESOLUTION.label);
    this.CIRCLE_RADIUS = this.RESOLUTION / 3;
    this.CIRCLE_DIAMETER = (this.RESOLUTION / 3) * 2;
    this.SQUARE_SIZE = this.RESOLUTION / 2;
  }
  ...
  setup = (p5, canvasParentRef) => {
    ...
    if (this.props.SHAPE_TYPE === 1) {
      p5.circle(p5.width / 2, p5.height / 2, this.CIRCLE_DIAMETER);
    } else if (this.props.SHAPE_TYPE === 2) {
      p5.rect(p5.width / 4, p5.height / 4, this.SQUARE_SIZE, this.SQUARE_SIZE);
    }
  };

  saveCanvas = () => {
    if (this.p5Instance) {
      this.p5Instance.saveCanvas(`generated_art-${this.SEED_STRING}`, "png");
    }
  };
      ...
      if (this.props.SHAPE_TYPE === 0) {
        ...
        if (
          p5.dist(
            ...
            this.points[i].x,
            this.points[i].y,
          ) < this.CIRCLE_DIAMETER
        ) {
          p5.ellipse(this.points[i].x, this.points[i].y, 1, p5.random(3));
        }
      } else if (this.props.SHAPE_TYPE === 2) {
        if (
          this.points[i].x > this.SQUARE_SIZE - this.SQUARE_SIZE / 2 + 2 &&
          this.points[i].x < this.SQUARE_SIZE + this.SQUARE_SIZE / 2 - 2 &&
          this.points[i].y > this.SQUARE_SIZE - this.SQUARE_SIZE / 2 + 2 &&
          this.points[i].y < this.SQUARE_SIZE + this.SQUARE_SIZE / 2 - 2
        ) {
          p5.ellipse(this.points[i].x, this.points[i].y, 1, p5.random(3));
        }
